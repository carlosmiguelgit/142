local combat = COMBAT_MANADRAIN
local effect = CONST_ME_LOSEENERGY
local effect2 = CONST_ME_MAGIC_BLUE
local disteffect = CONST_ANI_SMALLHOLY
local text_color = TEXTCOLOR_MAYABLUE 
local item = 14393
...
local function searchForTarget(cid)		
	...
	local hit_min = player:getLevel()*0.1 + 10
	local hit_max = player:getLevel()*0.2 + 10
